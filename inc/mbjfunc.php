...
{
		$query_mbjitem  = "SELECT category_name FROM mbj.category WHERE category_id = '" . htmlspecialchars($id, ENT_QUOTES) . "' LIMIT 1";
		$mbjitem = mysql_query($query_mbjitem);
	$row_mbjitem = mysql_fetch_assoc($mbjitem);
	
	return $row_mbjitem['category_name'];
	
}

function getActionAdminNameByID($id)
{
		$query_mbjitem  = "SELECT feedbacktype_name FROM mbj.feedback_type WHERE feedbacktype_id = '" . htmlspecialchars($id, ENT_QUOTES) . "' LIMIT 1";
		$mbjitem = mysql_query($query_mbjitem);
	$row_mbjitem = mysql_fetch_assoc($mbjitem);
	
	return $row_mbjitem['feedbacktype_name'];
	
}
function getLastFeedbackNoteByUserReportID($id)
{
	$query_mbjitem = "SELECT urf_note FROM mbj.user_reportfeedback WHERE urf_status = 1 AND userreport_id = '" . htmlspecialchars($id, ENT_QUOTES) . "' ORDER BY urf_date_y DESC, urf_date_m DESC, urf_date_d DESC, urf_id DESC LIMIT 1";
	$mbjitem = mysql_query($query_mbjitem);
	$row_mbjitem = mysql_fetch_assoc($mbjitem);

	return $row_mbjitem['urf_note'];
}
