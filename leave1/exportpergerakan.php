...
$objPHPExcel->getProperties()->setCreator("ISN Malaysia")
							 ->setTitle("Record Pergerakan Staff ISN");
...
while($row_userunit = mysql_fetch_assoc($userunit)){
	...
	while ( $day_num <= $days_in_month )
	{
		...
		{
			$objPHPExcel->setActiveSheetIndex(0)
				->setCellValueExplicit(toAlphabeth($day_num).$row_count, getPergerakanLocationByID($id, $day_str, $month, $year), PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->getActiveSheet()->getStyle(toAlphabeth($day_num).$row_count)
				->getAlignment()
				->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_FILL);
		}
