...
mysql_select_db($database_hrmsdb, $hrmsdb);
$query_hos = "SELECT * FROM hospital WHERE hospital_status = '1' ORDER BY hospital_name ASC";
$hos = mysql_query($query_hos, $hrmsdb) or die(mysql_error());
$row_hos = mysql_fetch_assoc($hos);
$totalRows_hos = mysql_num_rows($hos);
?>
...
?>
              ...
              <div id="sp_form" class="hidden">
                ...
                <li class="title">Butiran Perubahan Kew8<span class="fr add" onclick="toggleview2('formkeweayat'); return false;">Kemaskini</span></li>
                <div id="formkeweayat" class="hidden">
                <li>
                  <form id="keweayat" name="keweayat" method="post" action="<?php echo $url_main;?>sb/setting_admin.php">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td class="label noline">Jenis</td>
                        <td class="noline">
                        ...
                        <?php
							...
							$kewetype1 = mysql_query($query_kewetype, $hrmsdb) or die(mysql_error());
							$row_kewetype = mysql_fetch_assoc($kewetype1);
							$totalRows_kewetype = mysql_num_rows($kewetype1);
							?>
                          <select name="kewe_id" id="kewe_id" onchange="dochange('24', 'kewe_ayat1', this.value, '0');">
                          <option selected="selected" value="">Sila Pilih </option>
                            ...
                            <?php
                            mysql_select_db($database_hrmsdb, $hrmsdb);
                            $query_kewe1 = "SELECT * FROM kewe WHERE kewe_status = 1 AND kewetype_id='" . $row_kewetype['kewetype_id'] . "' ORDER BY kewetype_id ASC";
                            $kewe1 = mysql_query($query_kewe1, $hrmsdb) or die(mysql_error());
                            $row_kewe1 = mysql_fetch_assoc($kewe1);
                            $totalRows_kewe1 = mysql_num_rows($kewe1);
                            ?>
                            ...
                            <?php if ($totalRows_kewe1 > 0) { // Show if recordset not empty ?>
                              <?php
							do {  
							?>
                            <option value="<?php echo $row_kewe1['kewe_id']?>"><?php echo $row_kewe1['kewe_name']?></option>
                            <?php
							} while ($row_kewe1 = mysql_fetch_assoc($kewe1));
							  $rows1 = mysql_num_rows($kewe1);
							  if($rows > 0) {
								  mysql_data_seek($kewe1, 0);
								  $row_kewe1 = mysql_fetch_assoc($kewe1);
							  }
							?>
                          ...
                          <?php } // Show if recordset not empty ?>
    						<?php
							} while ($row_kewetype = mysql_fetch_assoc($kewetype1));
							  $rows = mysql_num_rows($kewetype1);
							  if($rows > 0) {
								  mysql_data_seek($kewetype1, 0);
								  $row_kewetype = mysql_fetch_assoc($kewetype1);
							  }
							?>
                          </select>                        
                      ...
                      <tr>
                        <td>&nbsp;</td>
                        <td name="kewe_ayat1" id="kewe_ayat1">
                          <!-- <textarea name="kewe_ayat" type="text" class="w50" id="kewe_ayat" cols="45" rows="6"></textarea> -->
                        </td>
                      </tr>
                      <tr>
                        <td>&nbsp;</td>
                        <td><input name="button4" type="submit" class="submitbutton" id="button4" value="Kemaskini" /></td>
                        <td><input type="hidden" name="MM_insert" value="keweayat" /></td>
                      </tr>
                    </table>
                  </form>
                </li>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      ...
                      <div class="off">
					  	<ul class="li2c">
                <?php do { ?>
              <h3><?php echo $row_kewetype['kewetype_name'] ?></h3><br/>

              <?php
                            mysql_select_db($database_hrmsdb, $hrmsdb);
                            $query_kewe2 = "SELECT * FROM kewe WHERE kewe_status = 1 AND kewetype_id='" . $row_kewetype['kewetype_id'] . "' ORDER BY kewetype_id ASC";
                            $kewe2 = mysql_query($query_kewe2, $hrmsdb) or die(mysql_error());
                            $row_kewe2 = mysql_fetch_assoc($kewe2);
                            $totalRows_kewe2 = mysql_num_rows($kewe2);
                            ?>
						  <?php do { ?>
					      <li><span class="name"><?php echo $row_kewe2['kewe_name']; ?> &nbsp; &bull; &nbsp; <?php echo $row_kewe2['kewe_desc']; ?></li>
						  <?php } while ($row_kewe2 = mysql_fetch_assoc($kewe2)); ?>
              <?php } while ($row_kewetype = mysql_fetch_assoc($kewetype1)); ?>
                        </ul>
                        </div>
                      </td>
                    </tr>
              ...
              <!-- baru 9/2/2021 -->
              <li class="title">Senarai Hospital<span class="fr add" onclick="toggleview2('hospital_form'); return false;">+ Tambah</span></li>
              <div id="hospital_form" class="hidden">
          	  <li>
          	    <form id="hospital" name="hospital" method="POST" action="<?php echo $url_main;?>sb/setting_admin.php">
          	      <table width="100%" border="0" cellspacing="0" cellpadding="0">
          	        <tr>
          	          <td nowrap="nowrap" class="label noline">Senarai Hospital</td>
          	          <td width="100%" class="noline">
       	              <input name="hospital_name" type="text" class="w50" id="hospital_name" />
       	              <input name="button" type="submit" class="submitbutton" id="button" value="Tambah" />
          	      <input type="hidden" name="MM_insert" value="hospital_submit" /></td>
       	            </tr>
                     <tr>
                        <td class="label">Negeri</td>
                        <td>
                ...
                <tr>
                  <td class="noline">
                    <ul class="li2c">
                                               <?php do { ?>
                   	    <li>
                        <span class="name"><?php echo $row_hos['hospital_name']; ?> , <?php echo getState($row_hos['state_id']);?></span>
                        <span class="del"><a onclick="return confirm('Anda mahu memadam maklumat berikut? \r\n\n<?php echo  $row_hos['hospital_name']; ?>')" href="<?php echo $url_main;?>sb/setting_admin.php?hid=<?php echo $row_hos['hospital_id'];?>">&times;</a></span> 
                        </li>

                          <?php } while ($row_hos= mysql_fetch_assoc($hos)); ?>
                    </ul>
                  </td>
                </tr>
              </table>
