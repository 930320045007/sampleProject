...
mysql_select_db($database_skt, $skt);
$query_uskt = sprintf("SELECT user_skt.* FROM skt.user_skt LEFT JOIN skt.pp ON pp.user_stafid = user_skt.user_stafid WHERE pp_ppk = '" . $row_user['user_stafid'] . "' AND uskt_id = %s LIMIT 1", GetSQLValueString($colname_uskt, "int"));
...
<div>
	<div>
        ...
        <div class="tabbox">
            ...
            <ul>
          <?php if(checkPPKByStafID($row_user['user_stafid']) && getPPKByStafID($row_uskt['user_stafid'])==$row_user['user_stafid']){?>
                <li class="form_back">
