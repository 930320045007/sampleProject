...
if(checkUserSysAcc($row_user['user_stafid'], 10, 81, 4))
{
	...
	{
	  $updateSQL = sprintf("UPDATE passenger SET passenger_status='0' WHERE transbook_id=%s AND passenger_id=%s",
	  ...
	  $updateGoTo .= "?msg=error&id=" . getID(htmlspecialchars($_GET['trid'], ENT_QUOTES)) . "&tid=" . getID(htmlspecialchars($_GET['id'],ENT_QUOTES));
  };
  
} else {
	$updateGoTo .= "?eul=1&id=" . getID(htmlspecialchars($_GET['trid'], ENT_QUOTES)) . "&tid=" . getID(htmlspecialchars($_GET['id'],ENT_QUOTES));
};
