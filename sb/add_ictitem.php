...
if(checkUserSysAcc($row_user['user_stafid'], 6, 26, 2))
{
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "formitem")) 
	{
		if(isset($_POST['itemnosiri']))
		{
		  ...
		  {
				$insertSQL = sprintf("INSERT INTO ict.item (item_by, item_date, item_isnsirihi, item_isnsiriyear, item_isnsiri, item_borrow, category_id, subcategory_id, brand_id, item_model, item_nosiri, item_price, item_getdate_d, item_getdate_m, item_getdate_y, item_nofile, item_voucher, item_warranty, warrantytype_id, vendor_id, item_app, item_appby, item_appdate) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
									 ...
									 GetSQLValueString($_POST['item_voucher'], "text"),
				...
				$insertGoTo = $url_main . "ict/item.php?msg=add";
		  };
		  
		} else {
			$insertGoTo = $url_main . "ict/item.php?e=1";
		};
		
	} else if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formitem")){
				$insertSQL = sprintf("UPDATE ict.item SET item_by=%s, item_date=%s, item_isnsirihi=%s, item_isnsiriyear=%s, item_isnsiri=%s, item_borrow=%s, category_id=%s, subcategory_id=%s, brand_id=%s, item_model=%s, item_nosiri=%s, item_price=%s, item_getdate_d=%s, item_getdate_m=%s, item_getdate_y=%s, item_nofile=%s, item_voucher=%s, item_warranty=%s, warrantytype_id=%s, vendor_id=%s WHERE item_id=%s",
									 GetSQLValueString($row_user['user_stafid'], "text"),
									 GetSQLValueString(date('d/m/Y h:i:s A'), "text"),
									 GetSQLValueString($_POST['item_isnsirihi'], "text"),
									 GetSQLValueString($_POST['item_isnsiriyear'], "text"),
				...
				$insertGoTo = $url_main . "ict/itemdetail.php?id=" . htmlspecialchars($_POST['item_id'], ENT_QUOTES) . "&msg=add";
	} else {
		$insertGoTo = $url_main . "ict/item.php?msg=error";
	};
	
} else {
	$insertGoTo = $url_main . "ict/item.php?eul=1";
};
