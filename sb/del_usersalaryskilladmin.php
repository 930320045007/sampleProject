...
if(checkUserSysAcc($row_user['user_stafid'], 5, 5, 4))
{
	...
	if ((isset($_GET['deltg'])) && ($_GET['deltg'] != "")) 
	{
		// $deleteSQL = sprintf("UPDATE user_salaryskill SET usersalaryskill_status = '0' WHERE usersalaryskill_id=%s AND user_stafid=%s",
		// 					 GetSQLValueString($_GET['deltg'], "int"),
		// 					 GetSQLValueString($colname_userprofile, "text"));

		$deleteSQL = sprintf("DELETE from user_salaryskill WHERE usersalaryskill_id=%s AND user_stafid=%s",
							 GetSQLValueString($_GET['deltg'], "int"),
							 GetSQLValueString($colname_userprofile, "text"));
