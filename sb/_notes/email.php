...
function emailset()
{
	...
	$smtpinfo["port"] = "587";
	$smtpinfo["auth"] = true;
	$smtpinfo["username"] = "hpt1test2017@gmail.com";
	$smtpinfo["password"] = "abcd1234??";
...
function activateEmail($to, $from, $subject, $type, $id)
{
	...
	if($from==0)
		$from = $GLOBALS['systitle_short'] . ' <hpt1test2017@gmail.com>';
...
Akaun email rasmi ISN boleh diakses pada URL berikut http://mail.isn.gov.my/ . Sila gunakan Kata Laluan Email sementara iaitu IsN123* . Perlu diingatkan bahawa kata laluan adalah sensitif, sila masukkan seperti mana yang dinyatakan dan menukar kepada kata laluan peribadi selepas mendaftar masuk.
...
Email rasmi ISN : <?php echo getEmailISNByUserID(htmlspecialchars($id, ENT_QUOTES));?>
...
<?php echo getDirSubName(getDirIDByMenuID(8));?> berhak membatal atau menukar maklumat pinjaman tanpa sebarang notis atau makluman. Pemohon bertanggungjawab sepenuhnya terhadap item dan kuantiti yang dipinjam sentiasa dalam keadaan baik dan berfungsi sepertimana keadaan sewaktu penerimaan.
...
Oleh 	: 
<?php echo getFullNameByStafID($to[0]) . " (" . $to[0] . "), Ext : " . getExtNoByUserID($to[0]);?>

<?php echo getFulldirectoryByUserID($to[0]);?>


<?php $menuid = 9; echo getDirSubName(getDirIDByMenuID($menuid));?> berhak untuk mengubah atau membatalkan maklumat tempahan tanpa notis atau makluman. Untuk maklumat lanjut, sila layari <?php echo $GLOBALS['systitle_short'];?> melalui URL berikut <?php echo $GLOBALS['url_main'];?>.
...
Catatan : 
<?php echo htmlspecialchars_decode(getLeaveNoteByLeaveOfficeID($id));?>
...
Kelulusan permohonan kebenaran meninggalkan pejabat dalam waktu pejabat (AM 5 BAB G) adalah <?php if(getHeadApprovalByLeaveOfficeID($id)=='1') echo "DILULUSKAN"; else echo "TIDAK DILULUSKAN";?> <?php if(checkWarningByLeaveOfficeID($id)) echo " dengan AMARAN ";?> oleh <?php echo getFullNameByStafID(getHeadApprovalByByLeaveOfficeID($id)) . " (" . getHeadApprovalByByLeaveOfficeID($id) . ")";?> pada <?php echo getHeadApprovalDateByLeaveOfficeID($id);?>.

Catatan 	:
<?php echo getHeadApprovalNoteByLeaveOfficeID($id);?>
...
Sebarang perubahan atau pembatalan berkiatan tempahan kenderaan, sila berhubung terus dengan <?php echo getDirSubName(getDirIDByMenuID(9));?>.
...
Berikut maklumat permohonan baru pinjaman peralatan <?php echo getDirSubName(getDirIDByMenuID(20));?> untuk tindakan selanjutnya :-
...
Oleh   : <?php echo getFullNameByStafID(getUserIDByUserBorrowID(htmlspecialchars($id, ENT_QUOTES))) . " (" . getUserIDByUserBorrowID(htmlspecialchars($id, ENT_QUOTES)) . "), Ext : " . getExtNoByUserID(getUserSportsIDByUserBorrowID(htmlspecialchars($id, ENT_QUOTES)));?>

Unit   : <?php echo getFulldirectoryByUserID(getUserSportsIDByUserBorrowID(htmlspecialchars($id, ENT_QUOTES)));?>
...
<?php 
$item2 = getSubCategoryItemByUserBorrowID(htmlspecialchars($id, ENT_QUOTES));
...
Makluman ini dihantar kepada <?php echo getDirSubName(getDirIDByMenuID(20));?> melalui <?php echo $GLOBALS['systitle_full'];?> sebagai rujukan.
<?
	//copy current buffer contents into $message variable and delete current output buffer
	$messageICT = ob_get_clean();
	}
