...
<?php
$insertGoTo = $url_main . "ict/index.php";
	...
	if($total>0)
	{
		...
		if(($dmy[0]>=date('d') && $dmy[1]== date('m') && $dmy[2]== date('Y')) || ($dmy[1]> date('m') && $dmy[2]=date('Y')) || ($dmy[2]>date('Y')))
		{
			...
			if(getTotalUserBorrowByDate($_POST['user_stafid'], $dmy[0], $dmy[1], $dmy[2])<=3)
			{
			  ...
			  $emailto = array_merge($emailto,getUserIDSysAcc(6, 27));//ICT yg ada kelulusan Modul
			  
			  emailPinjamanBaruICT($emailto, 0, 0, 1, $borrowid);

  			$insertGoTo  = $url_main . "ict/recorddetail.php?id=" . getID($borrowid) . "&msg=add";
